...
wk.register({
  ...
  r = { -- Add your rails folders and commands here.  SPC r + key
    j = { ":Neorg<CR>", "Toggle Neorg" },
    name = "+Rails",
    o = { ":lua require('lsp_lines').toggle()<CR>", "Toggle Lint" },
    O = { ":ToggleDiag<CR>", "Toggle Lint" },
    ...
    k = { ":%bd|e#|bd#<CR>", "Kill All but current" },
    K = { ":%bd<CR>", "Kill All" },
    U = { ":Efixtures<CR>", "Find Current Fixture" },
    N = { ":Emigration<CR>", "Find Current Migration" },
    t = { ":TransparentToggle<CR>", "Toggle trs window" }
  },
  w = {
    j = { ":WorkspacesOpen web-app<CR>", "Code Workspace"},
    k = { ":WorkspacesOpen vimwiki<CR>", "VimWiki Workspace"},
    w = { ":VimwikiIndex<CR>", "Vimwiki index" },
    e = { ":WindowsToggleAutowidth<CR>", "Toggle Autowidth" },
    E = { ":WindowsEqualize<CR>", "WindowsEqualize" },
  },
  s = {
    w = { ":lua require('telescope').extensions.live_grep_args.live_grep_args()<CR>", "search exact match" },
    t = { ":FloatermToggle<CR>", "Toggle Terminal"}
  },
  t = {
    t = { ":pu=strftime('%c')<CR>", "timestamp" }
  }
...
vim.g.folder_to_ignore = { ".*.git/.*", "node_modules/.*", "sorbet/.*", "public/.*", "coverage/.*", "tmp/.*", ".byebug_history", "vendor/*" } -- Ignore some folders on search?

-- Mouse?
vim.api.nvim_set_option('mouse', 'a')

set('relativenumber', false) -- relative numbers?
...
set('colorcolumn', '100') -- column length helper
